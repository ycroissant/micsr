...
bivprobit <- function(formula, data, weights, subset, na.action, offset, ...){
    ...
    dbivnorm <- function(x1, x2, rho)
        exp(- 0.5 * (x1 ^  2 + x2 ^ 2 - 2 * rho * x1 * x2) /
            (1 - rho ^ 2)) / (2 * pi * sqrt(1 - rho ^ 2))
    lnl <- function(param, gradient = FALSE, hessian = FALSE, sum = TRUE, robust = TRUE, X1, X2, q1, q2){
        ...
        r <- rho * q1 * q2
        PHI2 <- pbnorm(w1, w2, r)
        phi2 <- exp(- 0.5 * (w1 ^  2 + w2 ^ 2 - 2 * r * w1 * w2) / (1 - r ^ 2)) / (2 * pi * sqrt(1 - r ^ 2))
        result <- sum(log(PHI2))
        if (gradient){
        ...
        if (hessian){
            ...
            h_1r <- q2 * phi2 / PHI2 * (r * delta * v1 - w1 - g1 / PHI2) * hess_rho(v) * (-1)
            # pb de signe par rapport à Greene
            h_2r <- q1 * phi2 / PHI2 * (r * delta * v2 - w2 - g2 / PHI2) * hess_rho(v) * (-1)
            gradr <- q1 * q2 * phi2 / PHI2
            gradv <- gradr * grad_rho(v)
            h_rr <- phi2 / PHI2 * (delta ^ 2 * r * (1 - delta ^ 2 * (w1 ^ 2 + w2 ^ 2 - 2 * r * w1 * w2)) +
                                   delta ^  2 * w1 * w2 - phi2 / PHI2) * grad_rho(v) ^ 2 + gradr * hess_rho(v)
            H1 <- cbind(crossprod(h_11 * X1, X1), crossprod(h_12 * X1, X2), apply(h_1r * X1, 2, sum))
            H2 <- cbind(crossprod(h_12 * X2, X1), crossprod(h_22 * X2, X2), apply(h_2r * X2, 2, sum))
            Hr <- c(apply(h_1r * X1, 2, sum), apply(h_2r * X2, 2, sum), sum(h_rr))
            H <- rbind(H1, H2, Hr)
            attr(result, "hessian") <- H
        }
        result
    }
    f <- function(x, X1, X2, q1, q2) - lnl(x, gradient = FALSE, sum = TRUE, robust = TRUE, X1, X2, q1, q2)
    g <- function(x) - attr(lnl(x, gradient = TRUE, sum = TRUE, robust = TRUE, X1, X2, q1, q2), "gradient")
    h <- function(x) attr(lnl(x, gradient = FALSE, sum = TRUE, hessian = TRUE, robust = TRUE, X1, X2, q1, q2), "hessian")
    start_1 <- coef(pbt1)
    start_2 <- coef(pbt2)
    names(start_1) <- paste("eq1", colnames(X1), sep = "_")
    names(start_2) <- paste("eq2", colnames(X2), sep = "_")
    .start <- c(start_1, start_2, rho = 0)
    .coefs <- newton(lnl, .start, trace = FALSE, direction = "max", tol = 1E-06, X1 = X1, X2 = X2, q1 = q1, q2 = q2)
    ...
    lnl_null <- lnl(.coefs_null, gradient = FALSE, sum = TRUE, robust = TRUE, X1 = matrix(rep(1, N), ncol = 1), X2 = matrix(rep(1, N), ncol = 1), q1 = q1, q2 = q2)
    .coefs[K1 + K2 + 1] <- tan(.coefs[K1 + K2 + 1] * pi / 2)
    .lnl_conv <- lnl(.coefs, sum = FALSE, gradient = TRUE, hessian = TRUE, robust = FALSE, X1 = X1, X2 = X2, q1 = q1, q2 = q2)
